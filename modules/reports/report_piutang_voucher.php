...
$month = isset($_GET['month']) ? $_GET['month'] : ($defaultYear.'-'.$defaultMonth);
...
// total piutang dari pengajuan VF bulan ini
$q1 = $mysqli->prepare("SELECT COALESCE(SUM(total_saldo),0) FROM pengajuan_stock WHERE rs_type='vf' AND tanggal BETWEEN ? AND ?");
...
// total paid dari setoran harian bulan ini (asumsi semua paid terkait voucher)
...
// Left table: Inventory masuk (warehouses) + Pengajuan VF
$inventoryRows = [];
$sqlInv = "SELECT i.tanggal, p.nama_produk, i.jumlah AS qty, p.hpp_saldo AS hpp, (i.jumlah * p.hpp_saldo) AS total, p.kategori, c.nama_cabang, 'Stock Masuk' AS jenis
					 ...
					 WHERE i.tipe_transaksi='masuk' AND i.cabang_id IN ($warehouseIn) AND i.tanggal BETWEEN ? AND ?";
...
$pengajuanRows = [];
$sqlPeng = "SELECT ps.tanggal, p.nama_produk, psi.qty, psi.harga AS hpp, (psi.qty * psi.harga) AS total, p.kategori, c.nama_cabang, 'Pengajuan VF' AS jenis
						...
						WHERE ps.rs_type='vf' AND ps.tanggal BETWEEN ? AND ?
							AND o.jenis_rs = 'RS Eksekusi Voucher'";
...
// Right table: paid detail (setoran_harian)
$paidRows = [];
$sqlPaid = "SELECT sh.tanggal, sh.produk AS nama_produk, sh.qty, sh.harga_satuan AS hpp,
						...
						WHERE sh.tanggal BETWEEN ? AND ?
							AND p2.kategori IN ('Voucher Internet Lite','Voucher Internet ByU')";
...
<head>
	...
	<title>Report Piutang Voucher</title>
